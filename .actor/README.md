...
Both tiers share a single `AbortController`. The moment either tier returns a block verdict, ContextWall aborts the local poll loop **and** calls `client.run(id).abort()` to kill the upstream scraper run. You stop paying for the run and the toxic items never reach the agent. On a block, `data` is returned empty.
...
- **Architecture & diagrams** — [ARCHITECTURE.md](https://github.com/maxmedina05/context-wall-actor/blob/main/ARCHITECTURE.md)
- **Demo walkthrough** — [DEMO.md](https://github.com/maxmedina05/context-wall-actor/blob/main/DEMO.md)
- **Agent-poisoning demo** (real agent, poisoned vs protected) — [VICTIM-DEMO.md](https://github.com/maxmedina05/context-wall-actor/blob/main/VICTIM-DEMO.md)
- **Multi-source selective gate** — [MULTISOURCE-DEMO.md](https://github.com/maxmedina05/context-wall-actor/blob/main/MULTISOURCE-DEMO.md)
- **Live dashboard** — [a one-page visual readout of a 3-source run](https://maxmedina05.github.io/context-wall-actor/)
- **Source** — [github.com/maxmedina05/context-wall-actor](https://github.com/maxmedina05/context-wall-actor)
